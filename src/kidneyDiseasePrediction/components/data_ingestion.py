...
class DataIngestion:
    ...
    def download_file(self) -> str:
        ...
        try:
            ...
            logger.info(
                f"Downloading file from {dataset_url} to {zip_download_dir}"
            )

            drive_unique_file_id = dataset_url.split("/")[-2]

            download_prefix_url = "https://drive.google.com/uc?export=download&id="
            gdown.download(download_prefix_url + drive_unique_file_id,
                           zip_download_dir)

            logger.info(
                f"Downloaded file from {dataset_url} to {zip_download_dir}"
            )
